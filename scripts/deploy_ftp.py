#!/usr/bin/env python3
"""Deploy the Tutifrutsy static site over FTP/FTPS.

Default behavior:
  - uploads public static files from the project root
  - skips docs, deploy scripts, Git files, and local secrets
  - uploads only files that changed
  - does not delete remote-only files unless --prune is passed
"""
...
def load_config(args: argparse.Namespace) -> Config:
    ...
    host = args.host or env_value(values, "FTP_HOST", "ftp.tutifrutsy.com")
    username = args.username or env_value(values, "FTP_USERNAME", "admin@tutifrutsy.com")
    ...
    return Config(
        ...
        username=username,
        password=password,
        remote_dir=args.remote_dir or env_value(values, "FTP_REMOTE_DIR", "/tutifrutsy.com/public_html"),
        local_dir=local_dir,
...
def enter_remote_dir(ftp: ftplib.FTP, remote_dir: str) -> None:
...
def prune_remote(ftp: ftplib.FTP, local_dirs: list[str], local_files: list[str], protected: tuple[str, ...], dry_run: bool) -> None:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Deploy Tutifrutsy to SiteGround over FTP/FTPS.")
...
def main() -> int:
    ...
    ftp = connect(config)
    try:
        enter_remote_dir(ftp, config.remote_dir)
